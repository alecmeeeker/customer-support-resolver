...
class EnhancedEmailEmbeddings:
    ...
    def create_embedding_for_classified_email(self, email_id: int, classifications: List[str]):
        ...
        try:
            ...
            cursor.execute("SELECT * FROM classified_emails WHERE id = ?", (email_id,))
    ...
    def _get_or_create_sender_history(self, sender_email: str, sender_name: str) -> Dict:
        ...
        try:
            ...
            if history:
                ...
                """, (sender_email, sender_name, now_iso, now_iso, 1))
        ...
        try:
            ...
            if context:
                ...
                """, (
                    ...
                    json.dumps(thread_analysis['participants']),
                    json.dumps(thread_analysis['participant_names']),
                    thread_analysis['message_count'],
                    thread_analysis['our_message_count'],
                    thread_analysis['thread_type'],
                    thread_analysis['thread_status'],
                    thread_analysis['started_date'].isoformat() if isinstance(thread_analysis['started_date'], datetime) else thread_analysis['started_date'],
                    thread_analysis['last_activity_date'].isoformat() if isinstance(thread_analysis['last_activity_date'], datetime) else thread_analysis['last_activity_date'],
                    thread_analysis['summary'],
                    json.dumps(thread_analysis['key_topics'])
                ))
    ...
    def _get_related_articles(self, email_data: Dict, classifications: List[str]) -> List[Dict]:
        ...
        # Article tables (article_chunks/articles/outlets) and pgvector similarity
        # search are not available in the SQLite/LanceDB backend.
        logger.info("[ARTICLES] Article vector search not available in SQLite/LanceDB backend")
        return []
    ...
    def _create_comprehensive_embedding(self, email_data: Dict, sender_history: Dict,
        ...
        try:
            ...
            cursor.execute("""
                ...
                WHERE email_id = ? AND embedding_type = ?
            ...
            if existing:
                ...
                # Update existing (no embedding column in SQLite - vector lives in LanceDB)
                cursor.execute("""
                    UPDATE enhanced_email_embeddings SET
                        ...
                        includes_response = ?,
                        includes_thread_context = ?,
                        includes_sender_history = ?,
                        includes_pipeline_context = ?,
                        search_keywords = ?,
                        business_context = ?,
                ...
                """, (
                    ...
                    0,  # includes_response
                    1 if bool(thread_context) else 0,
                    1,  # includes_sender_history
                    1,  # includes_pipeline_context
                    json.dumps(thread_context.get('key_topics', []) if thread_context else []),
                ...
                # Update the vector in LanceDB
                try:
                    tbl = self.vector_db.open_table("enhanced_embedding_vectors")
                    tbl.delete(f"email_id = {int(email_data.get('id'))} AND embedding_type = 'comprehensive'")
                    tbl.add([{
                    ...
                    self.vector_db.create_table("enhanced_embedding_vectors", [{
                        "email_id": int(email_data.get('id')),
                        "embedding_type": "comprehensive",
                        "vector": embedding.tolist()
                    }], mode="overwrite")
            else:
                # Insert new (no embedding column in SQLite - vector lives in LanceDB)
                cursor.execute("""
                    INSERT INTO enhanced_email_embeddings (
                        email_id, embedding_type, embedding_text,
                ...
                """, (
                    ...
                    sender_history.get('total_emails_sent', 0),
                    thread_context.get('message_count', 0) if thread_context else 0,
                    0,  # includes_response (not yet)
                    1 if bool(thread_context) else 0,   # includes_thread_context
                    1,   # includes_sender_history
                    1,   # includes_pipeline_context
                    json.dumps(thread_context.get('key_topics', []) if thread_context else []),
                    f"Pipeline: {classifications[0] if classifications else 'unknown'}"
    ...
    def _store_pipeline_enrichment(self, email_id: int, classifications: List[str],
        ...
        try:
            ...
            cursor.execute("""
                INSERT OR REPLACE INTO pipeline_context_enrichment (
        ...
        try:
            ...
            cursor.execute("""
                ...
                SET total_emails_sent = total_emails_sent + 1,
                    ...
                    updated_at = datetime('now')
                WHERE sender_email = ?
            """, (date_sent, sender_email))

            # Update pipeline history if we have classifications
            if classifications:
                # Read current pipeline_history, append if not already present, write back as JSON
                cursor.execute("SELECT pipeline_history FROM sender_interaction_history WHERE sender_email = ?", (sender_email,))
                row = cursor.fetchone()
                if row:
                    current = json.loads(row[0] or '[]')
                    if classifications[0] not in current:
                        current.append(classifications[0])
                        cursor.execute("UPDATE sender_interaction_history SET pipeline_history = ? WHERE sender_email = ?",
                                      (json.dumps(current), sender_email))
    ...
    def search_enhanced_emails(self, query: str, limit: int = 20,
        ...
        try:
            ...
            # Add pipeline filter
            if pipeline_filter:
                sql += " AND eee.pipeline_classification = ?"
