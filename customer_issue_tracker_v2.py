...
{{
    ...
    def find_similar_resolved_issues(self, issue_embedding: np.ndarray, threshold: float = 0.75) -> List[Dict]:
        """Find similar issues that have been resolved using LanceDB vector search"""
        try:
            table = self.vector_db.open_table("issue_embedding_vectors")
            # Search for similar issue embeddings
            results = (table.search(issue_embedding.tolist())
                       .metric("cosine")
                       .limit(20)
                       .to_list())
        except Exception as e:
        ...
        # Filter by threshold and get issue details from SQLite
        similar_issues = []
        for r in results:
            similarity = 1.0 - r.get("_distance", 1.0)
            if similarity < threshold:
                continue
            if r.get("embedding_type") != "issue":
                continue

            issue_id = r["issue_id"]
            self.cursor.execute("""
                SELECT id, issue_type, issue_summary, resolution_summary, fix_instructions
                FROM customer_issues_v2
                WHERE id = ? AND fix_instructions IS NOT NULL
            """, (int(issue_id),))
...
{{
    ...
    def save_customer_issue(self, email_id: int, thread_id: str, analysis: Dict,
        ...
        try:
            ...
            # Insert relational data into SQLite (no vector columns)
            self.cursor.execute("""
            ...
            """, (
                ...
                1 if (has_resolution or fix_instructions is not None) else 0,
                resolution_summary,
                fix_instructions,
                similarity_score,
                json.dumps(based_on_issues) if based_on_issues else None,
                confidence_level
            ))
            ...
            issue_table = self.vector_db.open_table("issue_embedding_vectors")
            lance_rows = [{
                "issue_id": int(new_issue_id),
                "embedding_type": "issue",
                "vector": issue_embedding.tolist()
            }]
            if resolution_embedding is not None:
                lance_rows.append({
                    "issue_id": int(new_issue_id),
                    "embedding_type": "resolution",
                    "vector": resolution_embedding.tolist()
                })
            issue_table.add(lance_rows)

            logger.info(f"Saved customer issue for email {email_id} (confidence: {confidence_level})")

            # Cache similar issues
            if suggested_resolution and based_on_issues and new_issue_id:
                for similar_id in based_on_issues[:3]:
                    try:
                        self.cursor.execute("""
                            INSERT OR IGNORE INTO issue_similarity_cache
    ...
    def track_resolution_effectiveness(self, issue_id: int, was_effective: bool, feedback: str = None):
        ...
        self.cursor.execute("""
            ...
            VALUES (?, ?, ?)
        """, (issue_id, 1 if was_effective else 0, feedback))
    ...
    def get_issue_statistics(self) -> Dict:
        ...
        return {
            ...
            'resolution_rate': (summary['resolved_issues'] / summary['total_issues'] * 100) if summary['total_issues'] and summary['total_issues'] > 0 else 0
        }
    ...
    def export_fix_documentation(self, output_file: str = "customer_fixes_v2.json"):
        ...
        self.cursor.execute("""
            ...
            ORDER BY confidence_level DESC, similarity_score DESC
        """)
        ...
        for row in self.cursor.fetchall():
            ...
            fixes.append({
                ...
                'solution': row_dict['fix_instructions'],
...
def main():
    ...
    if args.stats:
        ...
        if stats['summary']['avg_similarity']:
            print(f"Avg Similarity Score: {stats['summary']['avg_similarity']:.3f}")
        ...
        try:
            table = tracker.vector_db.open_table("issue_embedding_vectors")
            results = table.search().where(f"issue_id = {args.test_similarity}").where("embedding_type = 'issue'").limit(1).to_list()
            if results:
                embedding = results[0]["vector"]
                tracker.cursor.execute("SELECT issue_summary FROM customer_issues_v2 WHERE id = ?", (args.test_similarity,))
                row = tracker.cursor.fetchone()
                if row:
                    print(f"\nTesting similarity for: {row['issue_summary']}")
                    similar = tracker.find_similar_resolved_issues(np.array(embedding), threshold=0.5)
